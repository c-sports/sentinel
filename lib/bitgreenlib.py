...
def is_valid_cspn_address(address, network='mainnet'):
    ...
    cspn_version = 98 if network == 'testnet' else 28
    ...
    try:
        ...
        # rescue from exception, not a valid CSPN address
        return False

    if (address_version != cspn_version):
        return False
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('cspn.conf')
