...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, cspnd):
        golist = cspnd.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_cspnd(cspnd, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_cspnd(self, cspnd, rec):
        import decimal
        import cspnlib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from cspnd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from cspnd, with every run
        ...
        try:
            ...
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(cspnd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from cspnd! %s" % e)
            govobj.vote_delete(cspnd)
            return (govobj, None)
    ...
    def vote_delete(self, cspnd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(cspnd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, cspnd, signal, outcome):
        ...
        output = cspnd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = cspnlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(cspnd, signal)

    def sync_network_vote(self, cspnd, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = cspnd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = CSPND_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 cspn addr, non-multisig
            if not cspnlib.is_valid_cspn_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid CSPN address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # CSPN Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (cspnlib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = CSPND_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not cspnlib.is_valid_cspn_address(addr, config.network):
    ...
    def hash(self):
        import cspnlib
        return cspnlib.hashit(self.serialise())
